...
describe("PreSell start campaign", () => {
    ...
    it("should fail to start", () => {
        return preSellDeploy(100, 100)
            .then(() => preSell.startCampaign(12)).should.be.rejected;
    })

    it("should fail to start", () => {
        return preSellDeploy(100, 100)
            .then(() => preSell.startCampaign('asd')).should.be.rejected;
    })

    it("should fail to start", () => {
        return preSellDeploy(100, 100)
            .then(() => preSell.startCampaign(1, {from: attacker})).should.be.rejected
    })

    it("should start campaign successfully", () => {
        return preSellDeploy(100, 100)
            .then(() => preSell.isCampaignStarted())
            .then(isCampaignStarted => assert.strictEqual(isCampaignStarted, false, "it should not be started yet"))
            .then(() => preSell.startCampaign(1, {from: owner}))
            .then(() => preSell.isCampaignStarted())
            .then(isCampaignStarted => assert.strictEqual(isCampaignStarted, true, "it should be started"))
    })

    it("should start campaign, stop it and then start it again", () => {
        return preSellDeploy(100, 100)
            .then(() => preSell.isCampaignStarted())
            .then(isCampaignStarted => assert.strictEqual(isCampaignStarted, false, "it should not be started yet"))
            .then(() => preSell.startCampaign(1, {from: owner}))
            ...
            .then(() => preSell.isCampaignStarted())
            .then(isCampaignStarted => assert.strictEqual(isCampaignStarted, false, "it should be stopped"))
            .then(() => preSell.startCampaign(1, {from: owner}))
            .then(() => preSell.isCampaignStarted())
            .then(isCampaignStarted => assert.strictEqual(isCampaignStarted, true, "it should be started again"))
    })
})
...
describe("PreSell token purchase", () => {
    ...
    it("should get token, stop campaign, restart the campaign and get more tokens", () => {
        return preSellDeploy(web3.toWei(1, "ether"), web3.toWei(10, "ether"))
            .then(() => preSell.isCampaignStarted())
            ...
            .then(() => preSell.startCampaign(1, {from: owner}))
            .then(() => preSell.isCampaignStarted())
            .then(isCampaignStarted => assert.strictEqual(isCampaignStarted, true, "should be started"))
            .then(() => web3.eth.sendTransactionPromise({
                from: tokenBuyer,
                to: preSell.address,
                value: web3.toWei(1, "ether"),
            ...
            .then(() => preSell.startCampaign(0, {from: owner}))
            .then(() => preSell.isCampaignStarted())
            .then(isCampaignStarted => assert.strictEqual(isCampaignStarted, false, "should be started"))
            .then(() => preSell.startCampaign(1, {from: owner}))
            .then(() => preSell.isCampaignStarted())
            .then(isCampaignStarted => assert.strictEqual(isCampaignStarted, true, "should be started"))
            .then(() => web3.eth.sendTransactionPromise({
